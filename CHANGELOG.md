...
## 12.9
- Docs: auditoria de consistencia tras v12.8 (criterio de `pesable`). `README.md` describia el trigger nuevo pero afirmaba "aplicado y verificado en `test_pos` y produccion" citando la fecha del criterio *anterior* (2026-07-30) -- reemplazado por una tabla explicita de estado por entorno (`test`=criterio nuevo, `test_pos`/`production`=pendiente). Corregidas 3 consultas SQL de auditoria en `README.md`/`documentos/DOCUMENTACION_ALMACENAMIENTO_PALOTEO.md` que todavia usaban `ind_permite_comandar`/categoria (darian resultados incorrectos si se corrieran hoy). `CLAUDE.md` seguia describiendo la derivacion vieja. `documentos/ux_pesaje_recomendaciones.md` segunia citando `nombre_ind_permite_comandar` como el campo que habilita los inputs de peso en el frontend (ahora es `catalogo_permite_pesar`).
- **Hallazgo durante la auditoria, sin resolver (ver `TODO.md`):** `HUARI 620ML` y `AMSTEL 620ML` (CERVEZAS) tienen `p_unidad_medida=11` en el catalogo real -- calificarian como pesables bajo el criterio nuevo, reabriendo sin decision de negocio el incidente de la operativa 1263 (2026-07-30, revertido en su momento) que fue precisamente el motivo original de excluir esa categoria. Pendiente verificar y decidir antes de re-aplicar los triggers 2026-09-09 en `test_pos`/`production`.
...
## 12.6
- Fix (PESAJE): `GET /api/pesaje/config` y `GET /api/pesaje/categorias` ya no excluyen las categorías de `CATEGORIAS_EXCLUIDAS_PESAJE` del listado — esa exclusión escondía perfiles reales (pesables y no pesables) en categorías como CERVEZAS tras el backfill que le agregó fila a todo el catálogo `HAB` en `app_producto_pesaje_config_api`.
- Test: cobertura de integración para `GET /api/pesaje/config` y `GET /api/pesaje/categorias` (gap señalado en revisión de PR #7, sin código de producción).
