...
- `config.py` — `pydantic-settings` `Settings` loaded from `.env`. Selects DB credentials based on `APP_ENV` (`test` = WAMP local, `production` = remote) via `settings.database_url`. Also owns paloteo "barra operativa" config (`PALOTEO_DEFAULT_BARRA_ID`, `PALOTEO_SELECTOR_ENABLED`, `PALOTEO_ALLOWED_BARRAS`) and, same allowlist pattern, `POURCOST_DIAS_PRECIO_ACTIVOS` (which `ope_dia` groups POUR COST exposes as real, in-use price schedules — see "Precios A/B" in `documentos/pour_cost/pourcost.md` section 8).
...
- **`vista_inventario_barra_con_filtro` is not pre-filtered by barra despite its name**: fixed 2026-09-08 (`CHANGELOG` 12.5, `TODO.md`) — `/api/inventario/pendientes` and `/api/inventario/catalogo/buscar` now add `AND i.id_barra = :id_barra` explicitly in the `JOIN`. Before that, a product with rows in `bar_inventario` for more than one barra (any real multi-barra deployment) produced one duplicate row per barra, which also multiplied every matching row from the `LEFT JOIN app_producto_pesaje_config_api` — the bottle-model selector in PALOTEO would show a single real profile duplicated. If you touch either of these two queries again, keep the `id_barra` filter; don't assume the view does it for you. Relatedly, `_calcular_diferencias_paloteo` (shared by consolidation preview/apply) had been referencing a column (`v.nro_barra`) from an older version of this same view that no longer exists — fixed to `v.id_barra` in the same pass.
...
- **Pesaje config is synced from `alm_producto` by DB triggers, not by this app**: `trg_alm_producto_after_insert`/`trg_alm_producto_after_update` on `alm_producto` (MySQL, external to this repo — no ORM/migration tracks them) keep `app_producto_pesaje_config_api.pesable`/`estado` aligned with the catalog, since the API has no hook into catalog changes made by the ERP/POS. Current definitions live in `querys/fix_trigger_alm_producto_after_insert.sql` and `querys/fix_trigger_alm_producto_after_update.sql` (source of truth — reapply manually if an environment is rebuilt). Full rationale, including the "phantom row" pitfall these fixes address, in `README.md` ("Triggers de base de datos") and `TODO.md` ("conflictos excepcionales de pesable"). Since v10.98, `PUT /api/pesaje/config/{id}` can also "promote" a `pesable=0` phantom row directly (`_producto_deberia_ser_pesable` re-checks the same catalog criteria as the triggers) — the two mechanisms are independent: triggers prevent new bad rows going forward, the endpoint fixes existing ones without raw SQL.
- **`pesable` derivation criterion changed 2026-09-09**: `alm_producto.p_unidad_medida IN (11, 61)` (`UNIDADES_MEDIDA_PESABLES` in `main.py`) replaced the previous `ind_permite_comandar=71 AND id_categoria NOT IN (10,11,13,14,15,17,18,19,20)` (`CATEGORIAS_EXCLUIDAS_PESAJE`, removed from the codebase). Updated together in that change: both triggers, `_producto_deberia_ser_pesable()`, the INCOMPLETOS synthetic block in `GET /api/pesaje/config`, and the frontend (`static/app.js` no longer derives eligibility from `nombre_ind_permite_comandar`; it reads `catalogo_permite_pesar`, a bool the API now computes and returns). **Per-environment trigger status is not uniform** — the SQL files always hold only the latest definition (each re-apply is `DROP TRIGGER IF EXISTS` + `CREATE TRIGGER`, no parallel versioning), but re-applying them to a live environment is a separate manual step from deploying this repo's code. Check `README.md` ("Triggers de base de datos" → "Estado de aplicación por entorno") before trusting which criterion a given environment's trigger actually uses today — do not assume it matches what's in `querys/` just because the app code does.
