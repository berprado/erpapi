...
**Nota aparte (`production` Beer Garden, no relacionada al criterio de `pesable`):** ambos triggers tambien sincronizan `app_producto_pesaje_config` (tabla legacy de un sistema propio de casa matriz, ver seccion completa mas abajo — verificado en vivo el 2026-09-10, activa ahi con datos reales). Esa tabla tenia un esquema desfasado en `test_pos` que rompia CUALQUIER `INSERT`/`UPDATE` sobre `alm_producto` — corregido con `querys/fix_esquema_legacy_app_producto_pesaje_config.sql` antes de poder verificar los triggers con un sanity check real. Repetir ese chequeo de esquema en produccion Beer Garden antes de re-aplicar los triggers ahi (`documentos/runbook_despliegue_pesaje_unidad_medida.md`, seccion 3.4) — casa matriz no lo necesita (esquema ya correcto, confirmado 2026-09-10).
