...
| `production` — casa matriz | 2026-07-30 (`ind_permite_comandar=71` + categoria excluida) — **pendiente re-aplicar** | 2026-07-30 — **pendiente re-aplicar** | `SHOW CREATE TRIGGER` (los dos), 2026-09-10 |
...
Los archivos `querys/fix_trigger_alm_producto_after_insert.sql` y `querys/fix_trigger_alm_producto_after_update.sql` contienen **la definicion 2026-09-09** (la mas reciente — cada re-aplicacion sobrescribe la anterior via `DROP TRIGGER IF EXISTS` + `CREATE TRIGGER`, no hay versionado paralelo). Correrlos contra `production` deja esos entornos al dia. Hasta que eso pase, un producto nuevo o modificado en esos entornos sigue derivando `pesable` con el criterio 2026-07-30 (categoria), no con el criterio 2026-09-09 (unidad de medida) — la app (`main.py`) ya asume el criterio nuevo en cuanto se despliegue el codigo de este cambio, independientemente de si el trigger de ese entorno ya fue actualizado: la unica pieza fuera de sync durante la ventana entre "deploy de codigo" y "re-aplicar trigger" es el trigger mismo, no la API.

**Nota aparte (`test_pos`/`production` Beer Garden, no relacionada al criterio de `pesable`):** ambos triggers tambien sincronizan `app_producto_pesaje_config` (tabla legacy de un sistema propio de casa matriz, ver seccion completa mas abajo). Esa tabla tenia un esquema desfasado en `test_pos` que rompia CUALQUIER `INSERT`/`UPDATE` sobre `alm_producto` — corregido con `querys/fix_esquema_legacy_app_producto_pesaje_config.sql` antes de poder verificar los triggers con un sanity check real. Repetir ese chequeo de esquema en produccion Beer Garden antes de re-aplicar los triggers ahi (`documentos/runbook_despliegue_pesaje_unidad_medida.md`, seccion 3.4).
