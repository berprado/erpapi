...
**Nota de vigencia (actualizado 2026-09-10):** las 3 consultas que siguen usan `p_unidad_medida IN (11, 61)` (`UNIDADES_MEDIDA_PESABLES`), el criterio vigente desde 2026-09-09. **Solo reflejan lo que corre en el entorno donde `trg_alm_producto_after_insert`/`after_update` ya fueron re-aplicados con esa definición** — al momento de escribir esto, `test` y `test_pos` (ver "Estado de aplicación por entorno" en "Triggers de base de datos" más abajo para el estado exacto y actualizado por entorno; no asumir que sigue igual sin mirar esa tabla). Contra un entorno que todavía tiene la versión 2026-07-30 del trigger (`production` al momento de escribir esto), reemplazar `p_unidad_medida IN (11,61)` por `ind_permite_comandar = 71 AND (id_categoria IS NULL OR id_categoria NOT IN (10,11,13,14,15,17,18,19,20))` para que la consulta refleje lo que ese entorno realmente deriva.
...
**⚠️ Estado de aplicacion por entorno — los dos scripts de abajo tienen DOS definiciones distintas en su historial, IMPORTANTE no asumir cual corre en cada entorno sin verificar, y verificar SIEMPRE los dos triggers por separado** (`after_insert` y `after_update` se instalan con comandos independientes — uno puede haber quedado en la version vieja aunque el otro ya este actualizado):

| Entorno | Version de `trg_alm_producto_after_insert` | Version de `trg_alm_producto_after_update` | Verificado con |
|---|---|---|---|
| `test` (WAMP local) | 2026-09-09 (`p_unidad_medida IN (11,61)`) | 2026-09-09 | `SHOW CREATE TRIGGER` (los dos) + INSERT/UPDATE de sanity check, 2026-09-09 |
| `test_pos` | 2026-09-09 | 2026-09-09 | `SHOW CREATE TRIGGER` (los dos) + INSERT/UPDATE de sanity check, 2026-09-10 (incluyo tambien el fix de esquema legacy de `app_producto_pesaje_config`, ver mas abajo) |
| `production` — casa matriz | 2026-09-09 | 2026-09-09 | `SHOW CREATE TRIGGER` (los dos) + INSERT/UPDATE de sanity check, 2026-09-10 |
| `production` — Beer Garden | Sin verificar directamente — se espera igual a `test_pos` antes del fix (misma linea, replica exacta) | Sin verificar directamente — idem | Pendiente verificar antes de aplicar nada ahi |

Los archivos `querys/fix_trigger_alm_producto_after_insert.sql` y `querys/fix_trigger_alm_producto_after_update.sql` contienen **la definicion 2026-09-09** (la mas reciente — cada re-aplicacion sobrescribe la anterior via `DROP TRIGGER IF EXISTS` + `CREATE TRIGGER`, no hay versionado paralelo). Solo falta correrlos contra `production` Beer Garden. Hasta que eso pase, un producto nuevo o modificado ahi sigue derivando `pesable` con el criterio 2026-07-30 (categoria), no con el criterio 2026-09-09 (unidad de medida) — la app (`main.py`) ya asume el criterio nuevo en cuanto se despliegue el codigo de este cambio, independientemente de si el trigger de ese entorno ya fue actualizado: la unica pieza fuera de sync durante la ventana entre "deploy de codigo" y "re-aplicar trigger" es el trigger mismo, no la API.

**Nota aparte (`production` Beer Garden, no relacionada al criterio de `pesable`):** ambos triggers tambien sincronizan `app_producto_pesaje_config` (tabla legacy de un sistema propio de **casa matriz** — activa ahi con 296 filas y datos reales, verificado en vivo el 2026-09-10). Esa misma tabla tenia un esquema desfasado en `test_pos` (linea Beer Garden) que rompia CUALQUIER `INSERT`/`UPDATE` sobre `alm_producto` alli — corregido con `querys/fix_esquema_legacy_app_producto_pesaje_config.sql` antes de poder verificar los triggers con un sanity check real. `production` Beer Garden **sigue sin verificar** este punto — repetir el chequeo de esquema (`documentos/runbook_despliegue_pesaje_unidad_medida.md`, seccion 3.4) ahi antes de re-aplicar los triggers, no asumir que ya esta bien. Casa matriz no necesita ese paso (esquema ya correcto ahi).

**Resultado de la auditoria en casa matriz (2026-09-10, tras aplicar el criterio nuevo):** 288 productos en el universo objetivo pesable, 63 sin ninguna fila de config (candidatos a aparecer en INCOMPLETOS) y **5 conflictos excepcionales** (catalogo dice pesable, la fila de config existente dice `pesable=0`) — entre ellos `HUARI 620ML` (id 138) y `AMSTEL 620ML` (id 466), los mismos productos del incidente de la operativa 1263 documentado en `TODO.md`. Ninguno de estos se edito por SQL directo (fabricar `peso_bruto` sin un pesaje fisico real seria peor que dejarlos incompletos) — la guarda de asimetria del trigger los mantiene en `pesable=0` hasta que alguien cargue el peso real via el modulo PESAJE ("promover"), no hay riesgo de que salten solos.
