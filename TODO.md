...
- [ ] **Igualar `bar_inventario` al fisico de forma incondicional en `/api/inventario/ajustes/aplicar`**
  - Motivacion: hoy el loop que iguala `bar_inventario` (`main.py:1653`, escritura en `main.py:1709-1710`) itera `deltas_con_diferencia`, o sea la lista **ya filtrada por tolerancia** (`main.py:1587-1589`). Un producto cuya unica diferencia este en onzas y caiga dentro de la banda de 0.5 nunca llega a esa actualizacion. Esto contradice el principio documentado ("`bar_inventario` debe quedar siempre igualado al fisico final", ver `documentos/decision_avance_delta_tolerancia_ajustes_pwa.md` seccion 5).
  - Por que no es un bug observable hoy: si el producto fue tolerado, su `delta_det_exacto` es `0.0` exacto (`real_det == ideal_det`), asi que escribir la fila seria un no-op. La equivalencia depende **enteramente** del invariante "todos los deltas son multiplos de 0.5" — el mismo invariante que justifica la banda de 0.5 (v10.39). Si el invariante se rompiera, el producto quedaria fuera del ajuste **y** con `bar_inventario` conservando el ideal viejo, divergiendo del fisico de forma silenciosa y arrastrandose entre operativas via `bar_inventario_cierre`.
  - Asimetria adicional a resolver: si el mismo producto tiene diferencia de botellas (`delta_paq`), si entra a `deltas_con_diferencia` y ahi se le escribe `cantidad_detalle = real_det` con la diferencia de onzas tolerada incluida. O sea que hoy el que una diferencia tolerada llegue o no a `bar_inventario` depende de si el producto ademas tiene diferencia de unidades.
  - Alternativa descartada (v10.71): corregir solo la redaccion de la documentacion para que describa el comportamiento real. Ya aplicado como parche temporal en `documentos/redondeo_y_tolerancia.md` y `CLAUDE.md`, pero deja el codigo sin garantizar lo que el principio promete.
  - Trabajo estimado: separar la igualacion de `bar_inventario` del filtro de ajustes — iterar `deltas` completo para igualar y seguir usando `deltas_con_diferencia` solo para generar movimientos. Ojo: `_validar_cardinalidad_bar_inventario` hoy se valida solo sobre los productos con diferencia; al ampliar el alcance de la escritura habria que ampliar tambien la validacion de cardinalidad, o el `UPDATE` podria fallar sobre un producto sin fila o con fila duplicada en `bar_inventario`.
  - **Es cambio de comportamiento en un endpoint que escribe al POS**: requiere validacion funcional end-to-end contra BD de test antes de produccion. Idealmente hacerlo despues del pendiente de tests automatizados de AJUSTES (abajo), para tener red de seguridad.
