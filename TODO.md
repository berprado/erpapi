...
- [ ] **Aplicar en `production` Beer Garden los cambios de PESAJE de la sesión 2026-09-08/10** (backfills + nuevo criterio de `pesable` por unidad de medida, PRs #7/#8/#9/#10): guía paso a paso completa en `documentos/runbook_despliegue_pesaje_unidad_medida.md`. `test_pos` y `production` casa matriz ya completados (2026-09-10, tras backup previo del usuario). Repetir el runbook completo (incluyendo el paso 3.4 de esquema legacy, que sí se espera necesario acá a diferencia de casa matriz).
...
- [x] **Resolver los "conflictos excepcionales" de pesable en PESAJE — causa raíz identificada (2026-07-30)**
  - Root cause real (no era un problema de datos aislado): `trg_alm_producto_after_insert` (trigger de BD sobre `alm_producto`, fuera del repo) crea automáticamente una fila "fantasma" en `app_producto_pesaje_config_api` (`nombre_perfil='Estándar'`) al insertar cualquier producto nuevo. En producción esa fila queda **siempre en `pesable=0`**, sin mirar `ind_permite_comandar`. Como `app_producto_pesaje_config_api` tiene una clave única real (`uk_producto_perfil` sobre `id_producto_almacen, nombre_perfil`), esa fila fantasma bloquea cualquier intento de arreglarlo desde la app: `POST /perfiles` choca con 409 (ya existe "Estándar"), `DELETE` rechaza con 400 (último perfil activo), y `PUT` con `pesable=0` solo permite editar `barcode`. La única salida hasta ahora era editar la BD directo — así se originó el bug de PATRON SILVER (ver v10.94 en CHANGELOG).
  - `test_pos` tenía una versión más nueva de ese trigger (deriva `pesable` de `ind_permite_comandar`, usa `NULL` en vez de `0`) más un `trg_alm_producto_after_update` que producción no tenía — pero su versión encontrada tenía el mismo bug de fondo (promovía `pesable=1` sin validar que el perfil tuviera datos reales), y además ninguna de las dos versiones excluía categorías (confirmado con el usuario: `ind_permite_comandar=71` no implica pesable — CERVEZAS/AGUAS Y JUGOS lo tienen sin ser pesables). **Ambos triggers corregidos y aplicados en producción y en `test_pos` (2026-07-30)**, ya excluyendo `CATEGORIAS_EXCLUIDAS_PESAJE` en el cálculo de `pesable`: `querys/fix_trigger_alm_producto_after_insert.sql`, `querys/fix_trigger_alm_producto_after_update.sql`. Verificado con `SHOW CREATE TRIGGER` en ambos entornos. Los dos entornos quedan ahora con la misma versión de trigger — no debería repetirse el patrón de fila fantasma para productos nuevos ni al actualizar el catálogo.
  - Alcance real en producción (auditado en vivo, no eran 4 sino **12 productos atascados**). **Resuelto en producción (2026-07-30) para 8 de los 12**: 7 VINOS (`CHANDON DEMI SEC 750ML`, `CODORNIU CLASICO 750ML`, `DUO TANNAT MERLOT 750ML`, `JUAN CRUZ 750ML`, `KOHLBERG BICENTENARIO 750ML`, `NAVARRO CORREAS EXTRA BRUT 750ML`, `PIONERO 750ML`) con el valor real ya validado en `test_pos` (`peso_bruto=5`, `tara=0`, `gramos_por_oz=1`), y `PATRON SILVER 750ML` destrabado a `pesable=1`/`NULL` (pendiente de pesar físicamente). Script: `querys/fix_12_productos_atascados_produccion.sql`.
  - **Los otros 4 (`AMSTEL 620ML`, `AMSTEL LATA 473ML`, `HUARI 620ML`, `-LIMONADA`) se aplicaron y se revirtieron el mismo día**: son categorías CERVEZAS/AGUAS Y JUGOS, excluidas a propósito del módulo PESAJE (`CATEGORIAS_EXCLUIDAS_PESAJE`, vigente hasta 2026-09-09). Al ponerlos en `pesable=1`, `AMSTEL LATA 473ML` (con movimiento real en la operativa 1263 activa en ese momento) empezó a pedir peso en vez de conteo por unidades en la captura en vivo — se detectó y revirtió de inmediato a `pesable=0`/`peso_bruto=0` (su estado original). ~~**Pendiente decisión de negocio**: ¿`ind_permite_comandar=71` en estos 4 es un dato de catálogo real (se pesan botellas grandes abiertas) o un error de carga? No reintentar el cambio sin resolver esto primero.~~ **Resuelto 2026-09-10**: se confirma que sí son pesables de verdad (botellas grandes abiertas) — el problema de 2026-07-30 fue de preparación operativa (nadie tenía el perfil de pesaje cargado ni la balanza lista en medio de una operativa en vivo), no de clasificación incorrecta. Ver alerta de abajo para las condiciones bajo las que se re-habilitan.
  - **✅ Decisión tomada (2026-09-10) sobre el cambio de criterio de `pesable`:** desde el 2026-09-09 `pesable` se deriva de `p_unidad_medida IN (11, 61)` en vez de categoría (ver CHANGELOG v12.8/v12.9, PR #9). Verificado en `test`: `HUARI 620ML` (id 117) y `AMSTEL 620ML` (id 137) tienen `p_unidad_medida=11` — bajo el criterio nuevo califican como pesables. Se acepta explícitamente: son pesables de verdad, se dejan promover sin excepción de código. Detalle técnico a tener en cuenta al desplegar (guía en `documentos/despliegue_seenode.md`): en `test` ninguno de los dos tiene fila en `app_producto_pesaje_config_api` todavía, así que el próximo `UPDATE` sobre `alm_producto` para cualquiera de los dos crea la fila directo en `pesable=1` vía `INSERT IGNORE` (`peso_bruto`/`tara`/`gramos_por_oz` en `NULL` — cae en INCOMPLETOS, no en un perfil roto). A diferencia de 2026-07-30, hoy `_procesar_items_paloteo` rechaza con `400` cualquier intento de pesar un perfil incompleto (fix v10.94) — no puede repetirse el "empezó a pedir peso en captura en vivo sin que nadie lo supiera" de la operativa 1263, en el peor caso el staff ve un error pidiendo completar el perfil antes de pesar. Aun así, recomendado completar el perfil real de estos 4 productos vía el módulo PESAJE apenas se re-aplique el trigger en cada entorno, antes de que caigan en una operativa real, para no sorprender al staff en medio de una jornada.
  - **Confirmado en `production` casa matriz (2026-09-10), tras aplicar el trigger nuevo**: `HUARI 620ML` (id 138) y `AMSTEL 620ML` (id 466) siguen exactamente en el estado post-incidente (`pesable=0`, `peso_bruto=0`) — la guarda de asimetría del trigger los protege de auto-promoverse (no tienen `peso_bruto`/`gramos_por_oz` reales cargados). Quedan pendientes de completarse vía PESAJE ("promover") — no se tocaron por SQL directo. La auditoría completa en casa matriz encontró 5 conflictos excepcionales en total (no solo estos 2) y 63 productos sin ninguna fila de config (candidatos a INCOMPLETOS) — ver `documentos/runbook_despliegue_pesaje_unidad_medida.md` para el detalle.
  - `ALMA TANNAT ROSADO 750ML` (296) y `MOSCOW MULE LATA` (478), los otros 2 del listado original de 4: el primero ya quedó bien resuelto en ambos entornos; el segundo sigue `pesable=0` sin resolver a propósito (decisión pendiente del usuario, sin urgencia porque no genera riesgo).

- [x] **Permitir "promover" un perfil `pesable=0` (fila fantasma) directo desde PESAJE, y guardar `peso_bruto` sin exigir `tara` junta — implementado v10.98 (2026-07-31)**
  - Resuelto: `PUT /api/pesaje/config/{id}` ya no exige `peso_bruto`/`tara` juntos, y ya no rechaza tocar un perfil en `pesable=0` si el catálogo (`ind_permite_comandar=71` y categoría fuera de `CATEGORIAS_EXCLUIDAS_PESAJE` — mismo criterio que los triggers, `_producto_deberia_ser_pesable` en `main.py`) dice que debería ser pesable. Con `peso_bruto` solo, el perfil pasa a `pesable=1` con `tara`/`gramos_por_oz` en `NULL` (INCOMPLETOS, editable normal); en VINOS se completa del todo en un solo paso (`tara=0`, `gramos_por_oz=1`). Si el catálogo no lo marca pesable, se sigue rechazando cualquier intento de tocar `peso_bruto`/`tara` — no repite el caso de AMSTEL/HUARI.
  - Frontend (`static/app.js`, `crearFilaPerfilPesaje`): los campos de peso ahora se muestran también para un perfil `pesable=0` cuando `nombre_ind_permite_comandar` dice que sí (no hace falta duplicar la exclusión de categorías del lado del cliente porque el listado ya la aplica antes de que el producto llegue al frontend). El guardado ya no exige que `tara` tenga un valor numérico para poder guardar.
  - Cobertura: `tests/test_integracion_pesaje.py` (6 tests) — promoción con solo `peso_bruto`, segunda edición completando `tara`, rechazo por categoría excluida, rechazo por `ind_permite_comandar` distinto de 71, VINOS en un solo paso, edición de solo `barcode` sin promover. Encontró y corrigió de paso un bug real: mezclar `float` (payload) con `Decimal` (columna ya guardada) rompía con `TypeError` al calcular `gramos_por_oz`, y un perfil recién promovido con ceros heredados (no `NULL`) los tomaba como "tara ya conocida" en vez de placeholder — ambos corregidos antes de tocar ningún entorno real.
  - Pendiente adicional detectado de paso (sin resolver, no bloqueante): en `test_pos`, además de `PATRON SILVER`, hay 4 productos más (`HUARI 620ML`, `-LIMONADA`, `AMSTEL 620ML`, `AMSTEL LATA 473ML`) con `pesable=1` pero `peso_bruto`/`gramos_por_oz` en `0` (no atascados, ya editables con el fix de hoy, solo sin completar).
...
- [x] **`vista_inventario_barra_con_filtro` y `vista_inventario_barra` tienen `bi.id_barra = 1` hardcodeado**
  - Motivacion original: ambas vistas (que alimentan `/api/inventario/pendientes` y `/api/inventario/catalogo/buscar`) tenian `WHERE ... AND bi.id_barra = 1` fijo en su definicion SQL, a pesar de que el nombre ("con_filtro") sugiere que deberian parametrizarse por barra. La relacion real producto↔barra vive en `bar_inventario` (id_producto + id_barra: 295 filas para barra 1, 44 para barra 2, 0 para barra 3) — `alm_producto.id_barra` es vestigial (solo 2/503 filas lo tienen seteado, sin relacion con el flujo real) y no debe usarse para esto.
  - **Resuelto (2026-09-08, PR #5).** La vista `vista_inventario_barra_con_filtro` ya no tiene `id_barra` hardcodeado en su definicion (se rediseño en algun punto para no filtrar por barra en absoluto, en vez de parametrizarse) — sin que el codigo de este repo se actualizara para compensarlo. Efecto real encontrado: con datos de mas de una barra en `bar_inventario` (multi-sucursal), un producto con movimiento en 2 barras generaba 2 filas en el `JOIN` de `/pendientes`/`/catalogo/buscar`, duplicando cada perfil de pesaje del `LEFT JOIN` siguiente (el selector de modelo de botella en PALOTEO aparecia con el mismo perfil repetido). Fix: `AND i.id_barra = :id_barra` explicito en el `JOIN` de ambos endpoints, usando el `id_barra_operativa` que ya se resolvia pero solo se usaba para validar el header, no para filtrar. `vista_inventario_barra` (la vista hermana mencionada aqui) ya no existe en la base (`Table doesn't exist` al consultarla) — no la consume ningun codigo de este repo.
  - De paso, en la misma sesion se encontro y corrigio un bug relacionado en `_calcular_diferencias_paloteo()` (usado por AJUSTES): referenciaba `v.nro_barra`, columna que tampoco existe ya en la vista actual (el mismo rediseño de esquema, sin actualizar el codigo) — causaba `500` en cualquier preview/aplicacion de ajustes. Ver CHANGELOG 12.5.
  - Verificado con `tests/test_integracion_ajustes.py` (16/16 en verde, antes fallaba 16/16 por la columna inexistente) y manualmente contra `adminerp_garden` con `PALOTEO_SELECTOR_ENABLED=true`/`PALOTEO_ALLOWED_BARRAS=1,2`.
