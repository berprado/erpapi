...
- [x] **Resolver los "conflictos excepcionales" de pesable en PESAJE — causa raíz identificada (2026-07-30)**
  ...
  - **Los otros 4 (`AMSTEL 620ML`, `AMSTEL LATA 473ML`, `HUARI 620ML`, `-LIMONADA`) se aplicaron y se revirtieron el mismo día**: son categorías CERVEZAS/AGUAS Y JUGOS, excluidas a propósito del módulo PESAJE (`CATEGORIAS_EXCLUIDAS_PESAJE`, vigente hasta 2026-09-09). Al ponerlos en `pesable=1`, `AMSTEL LATA 473ML` (con movimiento real en la operativa 1263 activa en ese momento) empezó a pedir peso en vez de conteo por unidades en la captura en vivo — se detectó y revirtió de inmediato a `pesable=0`/`peso_bruto=0` (su estado original). **Pendiente decisión de negocio**: ¿`ind_permite_comandar=71` en estos 4 es un dato de catálogo real (se pesan botellas grandes abiertas) o un error de carga? No reintentar el cambio sin resolver esto primero.
  - **⚠️ ALERTA (2026-09-10) — el cambio de criterio de `pesable` reabre este riesgo, sin que nadie lo decida:** desde el 2026-09-09 `pesable` se deriva de `p_unidad_medida IN (11, 61)` en vez de categoría (ver CHANGELOG v12.8, PR #9). Verificado en `test`: `HUARI 620ML` (id 117) y `AMSTEL 620ML` (id 137) tienen `p_unidad_medida=11` — bajo el criterio nuevo califican como pesables, sin que la pregunta de arriba se haya respondido nunca. Agravante: en `test` ninguno de los dos tiene fila en `app_producto_pesaje_config_api` todavía, así que el próximo `UPDATE` sobre `alm_producto` para cualquiera de los dos dispara `trg_alm_producto_after_update` → `INSERT IGNORE` crea la fila **directo en `pesable=1`**, sin pasar por la guarda de asimetría (esa guarda solo protege un `UPDATE` sobre una fila que ya existe, no el `INSERT` inicial) — reproduciría el incidente de la operativa 1263 en cualquier entorno donde esto no tenga fila todavía. **No re-aplicar los triggers 2026-09-09 en `test_pos`/`production` sin antes**: (1) verificar `p_unidad_medida` de estos 4 productos en cada entorno, (2) revisar el estado actual de su fila en `app_producto_pesaje_config_api` (existe vs no existe, `pesable` actual), y (3) decidir con el usuario si de verdad deben ser pesables ahora o si necesitan una excepción explícita (dato de catálogo corregido en el ERP, o una lista chica de IDs excluidos a nivel trigger/`main.py`).
