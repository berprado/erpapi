...
- [ ] **Validar end-to-end en test_pos la igualacion incondicional (v10.77) antes de produccion**
  - Es cambio de comportamiento en un endpoint que escribe al POS: ejercitar en test_pos (POS real, tunel localto.net) un ciclo completo de operativa con ajustes aplicados y confirmar cabeceras/detalles/igualacion/control como en los tests locales.
  - Antes del deploy, repetir en la BD destino el chequeo de duplicados HAB en `bar_inventario` (`GROUP BY id_barra, id_producto HAVING COUNT(*) > 1`): la validacion de cardinalidad ahora cubre tambien productos con diferencia tolerada, y un duplicado legacy cortaria el preview con 500.
