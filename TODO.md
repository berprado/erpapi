...
- [ ] **Permitir "promover" un perfil `pesable=0` (fila fantasma) directo desde PESAJE, y guardar `peso_bruto` sin exigir `tara` junta**
  - Motivación: hoy `PUT /api/pesaje/config/{id}` rechaza cualquier edición de `peso_bruto`/`tara` si `perfil.pesable == 0` (solo permite `barcode`), y tanto `POST` como `PUT` exigen `peso_bruto` y `tara` juntos. Esto fuerza a resolver perfiles "atascados" por SQL directo (ver ítem anterior) y no contempla el caso real: `peso_bruto` casi siempre se conoce de entrada, pero `tara` recién se puede medir cuando se termina el contenido de la botella.
  - Alcance propuesto (no implementado todavía): si el catálogo (`alm_producto.ind_permite_comandar=71`) dice que el producto debería ser pesable, permitir que `PUT` acepte `peso_bruto` solo (sin `tara`) y ponga `pesable=1`, dejando `tara`/`gramos_por_oz` en `NULL` hasta que se complete después con una segunda edición.
  - Pendiente adicional detectado de paso: en `test_pos`, además de `PATRON SILVER`, hay 4 productos más (`HUARI 620ML`, `-LIMONADA`, `AMSTEL 620ML`, `AMSTEL LATA 473ML`) con `pesable=1` pero `peso_bruto`/`gramos_por_oz` en `0` (no atascados, ya editables, solo sin completar).
