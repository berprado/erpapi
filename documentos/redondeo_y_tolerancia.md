...
- Backend: `_calcular_diferencias_paloteo` (`main.py:1317`) — fuente de verdad única compartida por el preview (`/api/inventario/consolidar/preview`, `main.py:1478`) y por aplicar ajustes (`/api/inventario/ajustes/aplicar`, `main.py:1606`). Es el **único** punto del sistema donde la tolerancia decide algo; en el resto (listado de pendientes, CONVERSOR) la `tolerancia_oz` sólo se reporta dentro del perfil.
- Sólo `delta_det_operativo` (no el exacto) se escribe en `bar_detalle_ajuste` / `bar_detalle_salida_inv` (`main.py:1707-1727`).
- La banda decide si el producto genera **movimientos documentales**: el filtro `deltas_con_diferencia` (`main.py:1607-1609`) descarta todo producto cuya única diferencia esté en onzas y caiga dentro de la banda. Si todos caen dentro, la respuesta es `status: "skipped"` y no se crea ninguna cabecera de ajuste/salida. Desde v10.77 la banda ya **no** decide la igualación de `bar_inventario` (ver abajo).
...
**Igualación de `bar_inventario` (incondicional desde v10.77):**
- Al aplicar ajustes, `bar_inventario` se iguala al físico exacto (`real_paq`/`real_det`, sin cuantizar) para **todo producto cuyo físico difiera del ideal** — la lista `deltas_a_igualar` (`main.py:1617-1619`), de la cual `deltas_con_diferencia` es subconjunto. El loop de igualación (`main.py:1736-1747`) está separado del filtro de tolerancia: un producto con delta dentro de la banda no genera documento, pero sí queda escrito al físico. Esas igualaciones extra quedan auditadas en `app_paloteo_ajuste_control.payload_json` (clave `igualaciones_sin_movimiento`) y avisadas en el `mensaje` de la respuesta.
- Antes de v10.77 el loop iteraba la lista ya filtrada por tolerancia, así que un producto tolerado nunca se escribía. Era un no-op mientras el invariante "todos los deltas son múltiplos de 0.5" se mantuviera (delta tolerado ⇒ `delta_det_exacto == 0.0`), pero una divergencia silenciosa si se rompía — con la asimetría de que un `delta_paq` concurrente sí arrastraba la escritura del físico completo. Ambas dependencias quedaron eliminadas.
- Límite de la garantía: si **ningún** producto genera movimientos, `/ajustes/aplicar` responde `skipped` sin escribir nada — no hubo consolidación (coherente con el preview, que reporta `skipped`, y con la PWA, que ni ofrece el botón de aplicar). La garantía es "toda consolidación aplicada deja `bar_inventario` igualado al físico de todos sus productos", no "el sistema converge sin aplicar".
- La validación de cardinalidad (`_validar_cardinalidad_bar_inventario`) se evalúa sobre el mismo conjunto `deltas_a_igualar` tanto en preview como en aplicar, para que un dato roto (producto sin fila o duplicado en `bar_inventario`) aparezca en el preview y no recién al confirmar.
- Cobertura automatizada: `tests/test_integracion_ajustes.py` (tolerado igualado sin movimiento, asimetría con `delta_paq`, todo-tolerado responde skipped, cardinalidad ampliada a tolerados).
